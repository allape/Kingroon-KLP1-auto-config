...
version_file="/Applications/UltiMaker Cura.app/Contents/MacOS/cura/CuraVersion.py"
cura_version=$(grep "CuraVersion" "$version_file" | head -1 | sed -En 's/.*([0-9]+\.[0-9]+)\.[0-9]+.*/\1/p')
config_files_folder="~/Library/Application Support/cura/$cura_version"
...
cp -R ./configs/configs/quality/kingroon_klp1 "$config_files_folder/quality/"
